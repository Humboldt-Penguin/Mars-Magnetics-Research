...
%% FUNCTIONS
% NOTE: next time just integrate these into the code, no point having them
% be independent functions. It also keeps me from having those horizontal
% lines that are so helpful for debugging. the only upside is that i ahve
% to run "clear" on all created variables, but that isn't that hard and is
% worth the benefit.
...
%         fprintf(1, "Reading %s:\n\n", mavenFiles(i_file).name)
        ...
        convert_magToShape_atLocation(fullFileName,targetFolder,h,downsample,clon,clat,Th);
    end 
