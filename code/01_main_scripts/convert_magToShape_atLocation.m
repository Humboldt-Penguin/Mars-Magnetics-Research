function convert_magToShape_atLocation(fullFileName,targetFolder,h,downsample,clon,clat,Th)


%%
% NOTE: since i cut out height/night filtering, this can just be broken
% down into local functions that exist in `process_batch.m`

% I'll pretty much end up rewriting this entire thing.

% I'll have to rewrite my own local loadpds for reduced files too, but that
% won't be bad. literally just a textscan for a bunch of floats
%%
...
v3:
    - Works with process_batch

Final version has been added to the end of the "process" script
%}

% displays datapoints before/after filters, but this doesn't work with parallel processing
verbose = false; 
if verbose
	fprintf(1, "writemagshape_v2.m is verbose")
end
...
[posX,posY,posZ,magX,magY,magZ,~,~,~,date] = loadpds(fullFileName);
...
temp = regexp(fullFileName,'\\','split');
temp = temp{end};
temp = regexp(temp,'\.','split');
newFilePath = fullfile(targetFolder, temp{1});
